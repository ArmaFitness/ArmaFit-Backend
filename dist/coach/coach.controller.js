"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CoachController = void 0;
const common_1 = require("@nestjs/common");
const coach_service_1 = require("./coach.service");
const guard_1 = require("../auth/guard");
const decorator_1 = require("../auth/decorator");
const add_athlete_dto_1 = require("./dto/add-athlete.dto");
let CoachController = class CoachController {
    constructor(service) {
        this.service = service;
    }
    getMyAthletes(req) {
        return this.service.getMyAthletes(req.user.id);
    }
    addAthlete(req, dto) {
        return this.service.addAthlete(req.user.id, dto);
    }
    removeAthlete(req, athleteId) {
        return this.service.removeAthlete(req.user.id, athleteId);
    }
    getAthleteWeightLogs(req, athleteId) {
        return this.service.getAthleteWeightLogs(req.user.id, athleteId);
    }
};
__decorate([
    (0, common_1.Get)('athletes'),
    __param(0, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], CoachController.prototype, "getMyAthletes", null);
__decorate([
    (0, common_1.Post)('athletes'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, add_athlete_dto_1.AddAthleteDto]),
    __metadata("design:returntype", void 0)
], CoachController.prototype, "addAthlete", null);
__decorate([
    (0, common_1.Delete)('athletes/:athleteId'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Param)('athleteId', common_1.ParseIntPipe)),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Number]),
    __metadata("design:returntype", void 0)
], CoachController.prototype, "removeAthlete", null);
__decorate([
    (0, common_1.Get)('athletes/:athleteId/weights'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Param)('athleteId', common_1.ParseIntPipe)),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Number]),
    __metadata("design:returntype", void 0)
], CoachController.prototype, "getAthleteWeightLogs", null);
CoachController = __decorate([
    (0, common_1.Controller)('coach'),
    (0, common_1.UseGuards)(guard_1.JwtGuard, guard_1.RolesGuard),
    (0, decorator_1.Roles)('coach'),
    __metadata("design:paramtypes", [coach_service_1.CoachService])
], CoachController);
exports.CoachController = CoachController;
//# sourceMappingURL=coach.controller.js.map